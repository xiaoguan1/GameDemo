// 链接：http://lua-users.org/lists/lua-l/2006-04/msg00162.html
...
#define PATH_MAX        4096	/* char is a path name including nul */
...
#define luaL_openlib(L, Name, func, cnt)	luaL_setfuncs(L, func, cnt)
...
{
	int n = luaL_optinteger(L, 1, errno);
...
static int Psetpid(lua_State *L)		/** setpid(option,...) */
{
	...
	switch (*what)
	{
		...
		case 'p':
		{
			pid_t pid  = luaL_checkinteger(L, 2);
			pid_t pgid = luaL_checkinteger(L, 3);
